...
def main():
    ...
    if configs.data_type == "online":
        def _generate_dataset(path, **configs):
            if torch.cuda.device_count() > 1:
                if dist.get_rank() == 0:
                    processed_dataset = [
                        generate_dataset(path, **configs, names="data/names.txt", entities="data/entities.txt")
                    ]
                else:
                ...
                dataset = generate_dataset(path, **configs, names="data/names.txt", entities="data/entities.txt")
            return dataset
    ...
    for epoch in range(configs.resume, configs.num_epochs):
        if (
            ...
            if configs.dataset["size"]["valid"] < 0: # if negative, resample from scratch
                _generate_dataset(configs.val_path, **configs_valid)
                base_dataset_valid = get_dataset(
                    configs.val_path, tokenizer, max_size=32 if configs.debug else 100000000
                )
            if not configs.only_eval and configs.dataset["size"]["train"] < 0: # if negative, resample from scratch
                _generate_dataset(configs.val_path, **configs_valid)
                base_dataset_train = get_dataset(
                    configs.train_path,
                    tokenizer,
